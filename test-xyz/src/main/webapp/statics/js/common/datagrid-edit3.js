...
function EditDg(index){		//element:table的id
	...
	if (typeof EditDg._initialized == 'undefined') {
		...
		EditDg.prototype.endEditing = function(resolve, reject){//结束编辑
			var _this = this;
			...
			if ($(_params.element[_this.index]).datagrid('validateRow', _this.editIndex)){
				...
				if(_this.checkData(row)){
					...
					if(!row){
						...
						delete row.newRow;
						pro = new Promise(
								(res, rej) => {_this.save(res, rej, _this.editIndex, row);}
							);
					}else{
					...
					if(pro == undefined){
						...
						pro.then(
								(res) => {if(resolve != undefined) resolve(res);},
								(rej) => {if(reject != undefined) reject(rej);}
							);
					}
				}else{
					_this.addEdit(row);
		...
		EditDg.prototype.checkSave = function(resolve, reject){//检查其他模块保存没，自动保存，保存不了返回false
			var _this = this;
			var pro = undefined;
			for (var i = 0; i < _params.num; i++) {
				...
				if(edit.editIndex != undefined){
					var pro = new Promise(
							(res, rej) => {edit.endEditing(res, rej);}
						);
				}
		...
		EditDg.prototype.add = function(row){//新增   row:{index: '', init:{}}
			var _this = this;
			new Promise(
					(res, rej) => {_this.checkSave(res, rej);}
				).then(
					(res) => {
						if(res){
							...
							$.messager.alert('错误','检查异常','error');
						}
					},
					(rej) => {$.messager.alert('错误','程序异常','error');}
				);
		};
		...
		EditDg.prototype.save = function(resolve, reject, index, row){//保存
			var _this = this;
			if(row){
				new Promise(
						(res, rej) => {
