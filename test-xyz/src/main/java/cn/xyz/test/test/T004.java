...
public class T004 {
	...
	public static void main(String[] args) {
		/*String words = "頭核心提示：澳大利亞FAXTS新聞3月5日刊登評論認為，美國在全球一系列被解釋成用來防禦來自伊朗和朝鮮導彈襲擊的彈道導彈防禦系統的部署行為，以及最近將先進具有反導能力愛國者導彈出售給台灣的動作，其根本目的是針對兩個主要的核大國--中國和俄羅斯。";
	...
	/*public static void main(String[] args) {
		...
		T004 converter = T004.getInstance(T004.SIMPLIFIED);
        //繁体转换简体
        ...
        String simplified = converter.convert(a);
        System.out.println(simplified);
        //简体转换繁体    
        String traditional = T004.convert(simplified,T004.TRADITIONAL);
	    ...
	    private static final T004[] converters = new T004[NUM_OF_CONVERTERS];
	     ...
	     * @return
	     */
	    public static T004 getInstance(int converterType) {
	        if (converterType >= 0 && converterType < NUM_OF_CONVERTERS) {
	            if (converters[converterType] == null) {
	                synchronized(T004.class) {
	                    if (converters[converterType] == null) {
	                        converters[converterType] = new T004(propertyFiles[converterType]);
	                    }
	                }
	            }
	    ...
	    public static String convert(String text, int converterType) {
	        T004 instance = getInstance(converterType);
	    ...
	    public String convert(String in) {
	        ...
	        for (int i = 0; i < in.length(); i++) {
	            ...
	            if (this.conflictingSets.contains(stackString.toString())) {
	            	//
	            } else if (this.charMap.containsKey(stackString.toString())) {
