...
function EditDg(index){		//element:table的id
	...
	if (typeof EditDg._initialized == 'undefined') {
		...
		EditDg.prototype.add = function(row){//新增   row:{index: '', init:{}}
			...
			let obj = row ? (row.init || {}) : {};
			let index = row ? row.index : undefined;
		...
		EditDg.prototype._add = function(index, row){//添加的方法
			let _this = this;
			row.newRow = true;
			index = index || $(_params.element[_this.index]).datagrid('getRows').length;
		...
		EditDg.prototype.copy = function(row){//复制:  row:{init:{},remove:[],retain:[]}  remove与retain不能同时用
			let _this = this;
			if(!_this.checkSave()) return;
			let _row = $(_params.element[_this.index]).datagrid('getSelected');
			if(_row){
				_this.addEdit(_row);
				let index = $(_params.element[_this.index]).datagrid('getRowIndex',_row);
				let obj = $.extend({},_row);
				let eds = $(_params.element[_this.index]).datagrid('options');
		...
		EditDg.prototype.clickRow = function(index, field, edit){//edit强制编辑//点击单元格             需要回调？
			...
			if(_this.isEdit && field){
				...
				let eds = $(_params.element[_this.index]).datagrid('options');
				for (let i = 0; i < eds.columns[0].length; i++) {//不用forEach,因为return会无效
				...
				if(eds.frozenColumns[0]){
					for (let i = 0; i < eds.frozenColumns[0].length; i++) {
		...
		EditDg.prototype.beginEdit = function(field){//开始编辑行，并聚焦
			...
			let rows = $(_params.element[_this.index]).datagrid('getRows');
			let row = rows[_this.editIndex];
			_this.originalRow = $.extend(true, {}, row);
			let t;
			let editor = $(_params.element[_this.index]).datagrid('getEditor', {index: _this.editIndex, field: field});
			if (editor){
				t = editor.target;
			} else {
				let editors = $(_params.element[_this.index]).datagrid('getEditors', _this.editIndex);
				if (editors.length){
					t = editors[0].target;
				}
			}
		...
		EditDg.prototype.save = function(index, row){//保存
			let _this = this;
			if(row){
				let para = ajaxParamData(_params.type, _params.save[_this.index], row);
		...
		EditDg.prototype.undo = function(){//取消编辑
			let _this = this;
			if(_this.editIndex != undefined){
				let row = $(_params.element[_this.index]).datagrid('getSelected');
				...
				/*$.messager.confirm('提示', '确定删除吗？', function(r){
					if (r){
						_this.removeBefore(_this, function(this2, _row){
							let para = ajaxParamData(_params.type, _params.remove[this2.index], _row);
					    	post(para, function(data){
					   			if(data.result){
					   				$.messager.show({
				...
				$.messager.confirm('提示', '删除后无法恢复，确定删除吗？', function(r){
					if (r){
						$.messager.confirm('提示', '请再次确定是否删除？', function(r){
							if (r){
								let para = ajaxParamData(_params.type, _params.remove[_this.index], row);
						    	post(para, function(data){
						   			if(data.result){
						   				$.messager.show({
						   					title: '信息', msg: data.msg, timeout: 3000, showType: 'slide',
			...
			if ($(_params.element[_this.index]).datagrid('validateRow', _this.editIndex)){
				...
				if(_this.checkData(row)){
					...
					if(!row){
						...
						let fields = $(_params.element[_this.index]).datagrid('getColumnFields',true).concat($(_params.element[_this.index]).datagrid('getColumnFields'));
						for(let i=0; i<fields.length; i++){
							let field = fields[i];
							let col = $(_params.element[_this.index]).datagrid('getColumnOption', field);
		...
		EditDg.prototype.reload = function(){//刷新数据
			...
			let length = _params.edit.length;
			if(length > 0){
			...
			let para = gridParamData(_params.type, _params.find[_this.index], row);
		...
		EditDg.prototype.download = function(){//下载         如果只用明细模块导出，需要修改参数getFormData
			let _this = this;
			let para = ajaxParamData(_params.type, _params.download[_this.index], getFormData(_params.form[_this.index]));
			post(para, function(data){
				if(data.result){
		...
		EditDg.prototype.task = function(_this, callback, time, row){//定时任务
			let i = 0;
			time = time || 100;
			let task_id = setInterval(function () {
				i++;
				if(i > 50){
